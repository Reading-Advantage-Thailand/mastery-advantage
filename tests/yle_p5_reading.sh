...
phase = plan[phase_matches[0].end():next_matches[0].start()] if phase_matches and next_matches else ""
if not re.search(r"^- \[b\][ \t]+Task: Curriculum plausibility review", phase, re.M):
    print("Phase 5 curriculum plausibility is not marked [b]", file=sys.stderr)
    raise SystemExit(1)
print("Phase 5 curriculum approval artifact:", "present" if approval_path.is_file() else "absent (human gate open)")
PY
)"
approval_status=$?
printf '%s\n' "$approval_output"
